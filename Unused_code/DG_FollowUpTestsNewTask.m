...
imageDir{2} = fullfile(homeDr,'filtered_scenes_objects_present_oldPar'); % image directory for old filter (1)
...
for i = 1:nTrials
    ...
    if paradigm{i,2} == 1
        ...
        nImgDir = 1; %new filter
    else
        paradigm{i,6} = imageDir{2};
        nImgDir = 2; %old filter
    end
...
DrawFormattedText(window, 'TRIAL INSTRUCTIONS\n\nIn each trial, you will be prompted with an object\n to look for within a scene.\n The prompted object will either be present \nin the scene, or a different object will be in the scene. \n\nPlease decide whether the prompted object is present in the scene.\n\n\nPress "f" for "YES" \nor\n "j" for "NO"\n\nPress any key to begin',...
...
for trl = 1:nTrials
    ...
    % 1. New filter, new task (i.e. objects present (Cond 1) or objects
    % absent (cond 2)
    % 2. Old fitler, new task
    ...
    while keypressed == 0
        ...
        if keyIsDown
            ...
            elseif keyCode(escapeKey) == 1
                ...
                savefilepath = fullfile(homeDr, 'Results', 'FollowUpTest_Data_NewTask',[savefilename '.mat']);
...
clear A1 A2 absentKey ans beepLengthSecs boxSize correct correctBeep...
    ...
    nTrials nTrialPerCon order pauseDur penWidth...
