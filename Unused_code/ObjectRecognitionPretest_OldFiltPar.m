...
% follow up tests for anyone who was trained on the radial checkerboard 
% filter that was not binarized.
% AKA participants: MC, YP, and GG
...
imageDir{1} = fullfile(homeDr, 'filtered_scenes_objects_present_oldPar');
imageDir{2} = fullfile(homeDr, 'filtered_scenes_objects_absent_oldPar');
...
%% trial variables
blocklength = 30; % number of trials per block
nblocks = 10;
ntrials = blocklength*nblocks; % THIS MUST BE IN MULTIPLES OF 3 BECAUSE THERE ARE 3 PRETEST CONDITIONS!!!
...
numPreTests = 3; % 1. one eye, 2. flip eye of orig 3. 1/f noise
...
testCond = repelem(tmp,(nblocks*blocklength)/3);
...
for trl = 1:ntrials
    ...
    switch cdt(trl, 3)
        case 1
            ...
            A2 = imread([imageDir{cdt(trl,1)} filesep fname{cdt(trl,1), cdt(trl,2)}(1:end-5), 'R.png']);
        ...
        case 3
            % Flipping L & R eyes
            A1 = imread([imageDir{cdt(trl,1)} filesep fname{cdt(trl,1), cdt(trl,2)}(1:end-5), 'R.png']);
            A2 = imread([imageDir{cdt(trl,1)} filesep fname{cdt(trl,1), cdt(trl,2)}]);
    end
    ...
    while keypressed == 0
        ...
        if keyIsDown
            ...
            elseif keyCode(escapeKey) == 1
                ...
                savefilepath = fullfile(homeDr, 'Results', 'PreTest_Data', [savefilename '.mat']);
