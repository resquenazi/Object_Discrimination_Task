...
imageDir{1} = [homeDr  filesep 'filtered_scenes_objects_present_newPar'];
imageDir{2} = [homeDr  filesep 'filtered_scenes_objects_absent_newPar'];
...
numPreTests = 1; % just regular training conditions (corresponds to CASE 1 in ObjectRecognitionPretest.m)
...
testCond = repelem(numPreTests,ntrials);
cdt = [cdt (testCond)'];
...
%%  Display name of prompting object
    ...
    % read in the L eye image corresponding to the correct condition and 
    % image number 
    A1 = imread(fname{cdt(trl,1), cdt(trl,2)});
    
    % read in the corresponding R eye image to A1
    ...
    while keypressed == 0
        ...
        if keyIsDown
            ...
            elseif keyCode(escapeKey) == 1 
                ...
                savefilepath = fullfile(homeDr, 'Results', 'Training_Data',[savefilename '.mat']);
    ...
    rObj(trl).preTestCond = cdt(trl,3); % corresponds to regular training condition 
