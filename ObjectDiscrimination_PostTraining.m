...
preTestCond = repelem(tmp,(nTrials)/4); % must be a multiple of 4 because there are 4 preTests
...
for i = 1:nTrials
    ...
    if paradigm{i,2} == 4 % for pinkNoise preTest condition
        nImgDir = 2;
    else % for all other preTest conditions
        nImgDir = 1; 
    end
...
for trl = 1:nTrials
    ...
    switch paradigm{trl,2}
        case 1 % Regular stimulus conditions

            % read in left eye image
            A1 = imread(fullfile(imageDir{1}, paradigm{trl,4}));
            
            % read in right eye image
            A2 = imread(fullfile(imageDir{1}, paradigm{trl,5}));
            
        case 2
            % Monocular viewing
            index = randi(2,1,(nTrials/4));
            for mon = 1:length(index)
                if index(mon) == 1
                    A1 = imread(fullfile(imageDir{1}, paradigm{trl,4}));
                    A2 = uint8(grey.*ones(800,800));
                else
                    A1 = uint8(grey.*ones(800,800));
                    A2 = imread(fullfile(imageDir{1}, paradigm{trl,5}));
                end
            end
        ...
        case 3
            ...
            A2 = imread(fullfile(imageDir{1}, paradigm{trl,4}));
            
        case 4
            % 1/f Noise
            A1 = imread(fullfile(imageDir{2}, paradigm{trl,4}));
            A2 = imread(fullfile(imageDir{2}, paradigm{trl,5}));
    end
